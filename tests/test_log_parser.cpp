...
// A throwaway trace directory: an STS file, one log file per PE, and a place
// for the pipeline to write its Parquet output. Removed on scope exit.
class TempTrace {
public:
  explicit TempTrace(const std::string &sts_contents) {
    root_ = std::filesystem::temp_directory_path() /
            ("charmvz_log_test_" + std::to_string(counter_++));
...
// NULL, so a test can assert on the file that was actually written rather than
// on an in-memory intermediate.
class ParquetTable {
public:
